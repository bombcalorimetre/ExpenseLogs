...
router.post('/register', (req, res) => {
  ...
  const user = db.prepare('SELECT id, name, email, base_currency, created_at FROM users WHERE id = ?').get(userId);
...
router.get('/me', authMiddleware, (req, res) => {
  const user = db.prepare('SELECT id, name, email, base_currency, fiscal_year_start, created_at FROM users WHERE id = ?').get(req.user.id);
...
router.put('/settings', authMiddleware, (req, res) => {
  ...
  const user = db.prepare('SELECT id, name, email, base_currency, fiscal_year_start FROM users WHERE id = ?').get(req.user.id);
  res.json(user);
});
